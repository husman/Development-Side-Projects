...
- (void)scrollViewDidScroll:(UIScrollView *)aScrollView {
    ...
    UIButton *targetButton = [headers objectAtIndex:i];
    UIView *accordion = targetButton.superview.superview;
    ...
    for (UIView *view in views) {
        if (self.isHorizontal) {
            ...
            if (view.frame.size.height > 0) {
                ...
                // If tag of accordion = 1, then flip the background image to yellow
                if(accordion.tag == 1) {
                    UIButton *btn = (UIButton *)header;
                    [btn setBackgroundImage:[UIImage imageNamed:@"M1.2A_Yellow_row_active.png"]
                ...
                // If tag of accordion = 1, then flip the background image to yellow
                if(accordion.tag == 1) {
                    UIButton *btn = (UIButton *)[headers objectAtIndex:i];
                    [btn setBackgroundImage:[UIImage imageNamed:@"M1.2A_Dk_Blue_row.png"]
                                   forState:UIControlStateNormal];
                    btn.selected = NO;
                }
            }
